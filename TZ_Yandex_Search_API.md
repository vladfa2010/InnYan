...
4. **Привяжи платежный аккаунт** к каталогу (нужно для Generative Search)
5. Стартовый грант: 4000 руб. на первые запросы

### Проверка работы ключа

Базовый поиск работает **без Folder ID**. Если запрос с Folder ID возвращает `403` — у ключа нет прав на этот каталог, но базовый поиск все равно будет работать.

Generative Search **требует Folder ID + платежный аккаунт**. Если ключ не проходит на `/v2/gen/search` — нужно проверить:
- Привязан ли платежный аккаунт к каталогу
- Есть ли у ключа роль `search-api.webSearch.user`
...
> **FolderId не обязателен** для базового поиска. Если добавить — убедись что у ключа есть права на этот каталог, иначе будет `403`.
...
| `query.queryText` | string | Да | Текст запроса, **max 400 символов** (жесткий лимит API Яндекса) |
| `folderId` | string | **Нет** | ID каталога Yandex Cloud. **Для базового поиска не обязателен** (работает и без него). Обязателен только для Generative Search |
...
| `403` | Нет прав `search-api.webSearch.user` для ключа; или у ключа нет доступа к указанному FolderId; или у каталога нет платежного аккаунта (для Generative Search) |
...
### Требования для Generative Search

В отличие от базового поиска, Generative Search **требует**:
1. **Folder ID** — обязателен в запросе
2. **Платежный аккаунт** — у каталога в Yandex Cloud должен быть привязан платежный аккаунт
3. **Права ключа** — ключ должен иметь роль `search-api.webSearch.user`

Если у ключа нет прав на folderId — запрос вернет `403 Permission denied`.
